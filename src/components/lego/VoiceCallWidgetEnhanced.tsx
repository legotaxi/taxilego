...
import { Phone, PhoneOff, Mic, MicOff, AlertCircle, Loader2, Volume2, VolumeX } from "lucide-react";
import { useVoiceCallEnhanced } from "@/hooks/use-voice-call-enhanced";

interface VoiceCallWidgetEnhancedProps {
  userId: string;
...
/**
 * Widget de chamada de voz melhorado - Interface para chamadas de voz em tempo real
 * Com melhor tratamento de áudio e UI responsiva
 */
export function VoiceCallWidgetEnhanced({
  userId,
  remoteUserId,
  ...
  // Reproduzir stream remoto com melhor controlo
  useEffect(() => {
  ...
  if (isCallActive) {
    return (
      <div className="fixed inset-0 z-50 bg-gradient-to-b from-blue-600 to-blue-800 flex flex-col items-center justify-center p-4">
        ...
        <div className="mb-8 flex h-32 w-32 items-center justify-center rounded-full bg-white/20 shadow-2xl ring-4 ring-white/30">
        ...
        {/* Nome e duração */}
        <h2 className="text-4xl font-bold text-white mb-2">{remoteUserName}</h2>
        <p className="text-lg text-blue-100 font-semibold">{formatDuration(callDuration)}</p>

        {/* Status de áudio */}
        {!isAudioPlaying && (
          <p className="text-sm text-blue-200 mt-2 animate-pulse">A conectar áudio...</p>
        )}
  ...
  if (showIncomingCall && isRinging) {
    return (
      <div className="fixed inset-0 z-50 bg-gradient-to-b from-emerald-600 to-emerald-800 flex flex-col items-center justify-center p-4">
        ...
        {/* Animação de toque */}
        <div className="mb-8 flex h-32 w-32 items-center justify-center rounded-full bg-white/20 shadow-2xl ring-4 ring-white/30 animate-pulse">
          <div className="text-6xl font-bold text-white">
            {remoteUserName.charAt(0).toUpperCase()}
          </div>
        </div>
  ...
  if (isRinging && !isCallActive) {
    return (
      <div className="fixed inset-0 z-50 bg-gradient-to-b from-blue-600 to-blue-800 flex flex-col items-center justify-center p-4">
        ...
        {/* Status da chamada */}
        <p className="text-sm text-blue-200 mt-4 capitalize">
          Estado: {callStatus}
        </p>
